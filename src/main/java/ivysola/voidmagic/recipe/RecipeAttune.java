    ...
    public boolean matches(IInventory inventory, World world, EntityPlayer player)
    {
        ...
        for (int i=0; i<3; i++)
        {
            for (int j=0; j<3; j++)
            {
                ...
                if (matrix != null && checkItemEquals(slotStack, new ItemStack(VoidItems.itemMatrix)))
                    return false;
                else if (checkItemEquals(slotStack, new ItemStack(VoidItems.itemMatrix)))
                    matrix = slotStack;
            }
        }
        if (matrix == null)
        ...
        for (int i=0; i<3; i++)
        {
            for (int j=0; j<3; j++)
            {
                ...
                if (slotStack == null)
                    continue;
                else if (slotStack.getItem().equals(VoidItems.itemMatrix))
                    continue;
        ...
        if (aspectMap.size() > 0)
        {
            ...
            output = new ItemStack(VoidItems.itemMatrix);
